...
starting_file_number = 100
num_of_files_each_test = 121                            # Please Specify How many i2c files are in each test
clock_speed = 3200e6                                       # Please specify your MACs clock rate
shunt_r = 24.4                                             # for low speeds since each step doesnt cause that much decrease in due to shunr_res, the supply stepping will be low and you cannot correct it. In that case increase the shunt_res to 20.
                                                        # for high speeds each freq step will result in a good amount of drop on the shunt_res and you can correct common mode by stepping up the supply. In that case reducing the shunt_res (8ohms) is good
                                                        # becase the sunt_res dynamic range should be kept below 80mV for PMIC to sense it.
...
#def save_result(path,file_name,head_iteration,i2c_call_num,head_res1,result1,head_res2,result2):    
    ...
    with open(results_dir, mode='a', newline='') as csvfile:
        ...
        if not file_exists:
            data_header = [head_iteration,head_res1,head_res2,head_res3,head_res4,head_res5,head_res6,head_res7,head_res8,head_res9,head_res10,head_res11,head_res12,head_res13,head_res14,head_res15,head_res16]
#            data_header = [head_iteration,head_res1,head_res2]
            writer.writerow(data_header)
        data =  ([i2c_call_num]) + ([result1]) + ([result2]) + ([result3])+ ([result4])+ ([result5])+ ([result6])+ ([result7])+ ([result8])+ ([result9])+ ([result10])+ ([result11])+ ([result12])+ ([result13])+ ([result14])+ ([result15])+ ([result16])                       # for additional result columns, you can add result2,result3, ... to the end
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range (starting_mac,(starting_mac+num_of_macs)):
            ...
            i2c_path = os.getcwd()
            i2c_path = os.path.join(i2c_path,'i2c_files')
            i2c_path = os.path.join(i2c_path,'speed_mthd4')        #specify what kind of test you are running
            i2c_path = os.path.join(i2c_path,str(i))

        #     dir1 = os.path.join(i2c_path,'MacL_UpL900')               # measurement name
        #     dir1 = os.path.join(i2c_path,'MacL_UpL550')               # measurement name
        #     dir1 = os.path.join(i2c_path,'MacL_UpL600')               # measurement name
            dir1 = os.path.join(i2c_path,'MacL_UpL700')               # measurement name            
        #     dir1 = os.path.join(i2c_path,'MacL_UpL800')
        #     dir1 = os.path.join(i2c_path,'MacL_UpL')               # measurement name         
            ...
            for z, directory in enumerate(directories):
                ...
                for n in range(starting_file_number,num_of_files_each_test):        ############for testing########
                    rst = []                        
                    sda = []
                    scl = []
                    ...
                    v_shunt = chp_pwr.vshunt()          # voltage on the sense resistor to PMIC (devide this by 20 and that is your current consumption)
                    save_result(directories[z],'results.csv','iteration',n,'MAC_ACT_POS_L',actp_l['r'],'MAC_ACT_NEG_L',actn_l['r'],'MAC_WGHT_POS_L',wghtp_l['r'],'MAC_WGHT_NEG_L',wghtn_l['r'],'MAC_ACT_POS_R',actp_r['r'],'MAC_ACT_NEG_R',actn_r['r'],'MAC_WGHT_POS_R',wghtp_r['r'],'MAC_WGHT_NEG_R',wghtn_r['r'],'chp_current(A)',i_chip,'chp_vshunt(V)',v_shunt,'chp_supply(mV)',chp_vdd['r'],'chp_vss(mV)',chp_vss['r'],'DMON_State',D_state,'CK_Speed',clock_speed,'Shunt_Res_PMIC',shunt_r,'AVG_NOISE',avg_noise['r'])
                    rst_amon()
                    print('...................................................')
