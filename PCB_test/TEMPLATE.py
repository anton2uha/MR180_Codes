...
i2c_path = os.path.join(i2c_path,'new_dac2')          # This is the Test Directory Name - Please Specify for each individual test
num_files = 6                                        # Please Specify How many i2c files are in that directory
...
##################### Save Func - csv ###########################################################################
def save_result(path,file_name,head_iteration,i2c_call_num,head_res1,result1,head_res2,result2,head_res3,result3,head_res4,result4):               # for additional result columns, you can add result2,result3, ... to the end
    cur_dir = os.getcwd()
    ...
    with open(results_dir, mode='a', newline='') as csvfile:
        ...
        if not file_exists:
            data_header = [head_iteration,head_res1,head_res2,head_res3,head_res4]
            writer.writerow(data_header)
        data =  ([i2c_call_num]) + ([result1]) + ([result2]) + ([result3])+ ([result4])                       # for additional result columns, you can add result2,result3, ... to the end
...
if __name__ == "__main__":
    ...
    try:
        ...
        for n in range(0,num_files):        ############for testing########
            ...
            send_i2c_file_2pi(rst,sda,scl)
            print("..........Data Was Sent.........")
            res1 = chp_adc.read_amon()
            print ("AMON:%fmV"%(res1['r']))
            res2 = chp_pwr.iin()            # current consumption
            save_result(i2c_path,'results.csv','iteration',n,'DAC3',res1['r'],'chp_current(A)',res2,'chp_supply(mV)',chp_vdd['r'],'chp_vss(mV)',chp_vss['r'])
            print('...................................................')
        chp_vdd = mon_vdd()
        chp_vss = mon_vss()
        chp_dac1 = mon_dac1_macR_actP()
        chp_dac3 = mon_dac3_macR_wghtP()
