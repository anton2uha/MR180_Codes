...
i2c_path = os.path.join(i2c_path,'mac1_offset_R_w1')          # This is the Test Directory Name - Please Specify for each individual test
num_files = 341                                       # Please Specify How many i2c files are in that directory
...
##################### Save Func - csv ###########################################################################
def save_result(path,file_name,head_iteration,i2c_call_num,head_res1,result1,head_res2,result2,head_res3,result3,head_res4,result4,head_res5,result5,head_res6,result6,head_res7,result7,head_res8,result8):               # for additional result columns, you can add result2,result3, ... to the end
#def save_result(path,file_name,head_iteration,i2c_call_num,head_res1,result1,head_res2,result2):    
    ...
    with open(results_dir, mode='a', newline='') as csvfile:
        ...
        if not file_exists:
            data_header = [head_iteration,head_res1,head_res2,head_res3,head_res4,head_res5,head_res6,head_res7,head_res8]
#            data_header = [head_iteration,head_res1,head_res2]
            writer.writerow(data_header)
        data =  ([i2c_call_num]) + ([result1]) + ([result2]) + ([result3])+ ([result4])+ ([result5])+ ([result6])+ ([result7])+ ([result8])                       # for additional result columns, you can add result2,result3, ... to the end
...
if __name__ == "__main__":
    ...
    try:
        ...
        for n in range(0,num_files):        ############for testing########
            rst = []                        
            sda = []
            scl = []
            ...
            path = os.path.join(i2c_path,fname)
        ...
        #         initial_DMON = D_state
                # mon_dac1_macR_actP()
                # actp  = chp_adc.read_amon()
                # mon_dac0_macR_actN()
                # actn  = chp_adc.read_amon()
                # mon_dac3_macR_wghtP()
                # wghtp  = chp_adc.read_amon()
                # mon_dac2_macR_wghtN()  
                # wghtn  = chp_adc.read_amon()                        #######this was for MAC_test
                # i_chip = chp_pwr.iin()            # current consumption
                # save_result(i2c_path,'results.csv','iteration',n,'MAC_ACT_POS',actp['r'],'MAC_ACT_NEG',actn['r'],'MAC_WGHT_POS',wghtp['r'],'MAC_WGHT_NEG',wghtn['r'],'chp_current(A)',i_chip,'chp_supply(mV)',chp_vdd['r'],'chp_vss(mV)',chp_vss['r'],'DMON_State',D_state)
                # rst_amon()
        #     if D_state != initial_DMON:
        #         initial_DMON = D_state  
        #         mon_dac1_macR_actP()
        #         actp  = chp_adc.read_amon()
        #         mon_dac0_macR_actN()
        ...
        #         save_result(i2c_path,'results.csv','iteration',n,'MAC_ACT_POS',actp['r'],'MAC_ACT_NEG',actn['r'],'MAC_WGHT_POS',wghtp['r'],'MAC_WGHT_NEG',wghtn['r'],'chp_current(A)',i_chip,'chp_supply(mV)',chp_vdd['r'],'chp_vss(mV)',chp_vss['r'],'DMON_State',D_state)
        #         rst_amon()
#            save_result(i2c_path,'results.csv','iteration',n,'MACL_ACT_POS',macl_act_pos['r'],'chp_current(A)',i_chip,'chp_supply(mV)',chp_vdd['r'],'chp_vss(mV)',chp_vss['r'],'DMON_State',D_state)
#            save_result(i2c_path,'results.csv','iteration',n,'chp_current(A)',i_chip,'DMON_State',D_state)
            initial_DMON = D_state
            mon_dac1_macR_actP()
            actp  = chp_adc.read_amon()
            mon_dac0_macR_actN()
