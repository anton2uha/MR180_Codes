...
num_of_files_each_test = 228                            # Please Specify How many i2c files are in each test
...
#     GPIO.setup(dmon_lcas, GPIO.IN, pull_up_down=GPIO.PUD_OFF)
    GPIO.setup(dmon_lcas, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)
...
def DMON_read():
    ...
    if GPIO.input(dmon_lcas) == GPIO.LOW:
        #print("DMON is 0")
        state = 0
    elif GPIO.input(dmon_lcas) == GPIO.HIGH:
        #print("DMON is 1")
        state = 1
    return state
##################### DMON - Read ###############################################################################
...
#def save_result(path,file_name,head_iteration,i2c_call_num,head_res1,result1,head_res2,result2):    
    ...
    with open(results_dir, mode='a', newline='') as csvfile:
        ...
        if not file_exists:
            data_header = [head_iteration,head_res1,head_res2,head_res3,head_res4,head_res5,head_res6,head_res7,head_res8,head_res9,head_res10,head_res11,head_res12]
#            data_header = [head_iteration,head_res1,head_res2]
            writer.writerow(data_header)
        data =  ([i2c_call_num]) + ([result1]) + ([result2]) + ([result3])+ ([result4])+ ([result5])+ ([result6])+ ([result7])+ ([result8])+ ([result9])+ ([result10])+ ([result11])+ ([result12])                       # for additional result columns, you can add result2,result3, ... to the end
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range (starting_mac,(starting_mac+num_of_macs)):
            ...
            i2c_path = os.getcwd()
            i2c_path = os.path.join(i2c_path,'i2c_files')
            i2c_path = os.path.join(i2c_path,'lin_mthd1')        #specify what kind of test you are running
            i2c_path = os.path.join(i2c_path,str(i))

            dir1 = os.path.join(i2c_path,'Mac_UpL')               # lin test name
            dir2 = os.path.join(i2c_path,'Mac_UpR')               # lin test name
            dir3 = os.path.join(i2c_path,'Mac_Down')               # lin test name
                       
        #     directories = [dir1,dir2,dir3]                       # making a list of those names
            directories = [dir1]                       ############# DANGER ################## DANGER ################ DANGER ############## DANGER ##########
            ...
            for z, directory in enumerate(directories):
                ...
                for n in range(0,num_of_files_each_test):        ############for testing########
                    ...
                    send_i2c_file_2pi(rst,sda,scl)
                    print("..........Data Was Sent.........")
                    time.sleep(2)                                       ####################### IMPORTANT ###################### DMON when reset is HIGH and could be still high after new data loaded...so give time       
                    D_state = DMON_read()
