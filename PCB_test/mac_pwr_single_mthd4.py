...
num_of_files_each_test = 18                            # Please Specify How many i2c files are in each test
clock_speed = 500e6                                       # Please specify your MACs clock rate
...
#def save_result(path,file_name,head_iteration,i2c_call_num,head_res1,result1,head_res2,result2):    
    ...
    with open(results_dir, mode='a', newline='') as csvfile:
        ...
        if not file_exists:
            data_header = [head_iteration,head_res1,head_res2,head_res3,head_res4,head_res5,head_res6,head_res7,head_res8,head_res9,head_res10,head_res11,head_res12,head_res13,head_res14,head_res15]
#            data_header = [head_iteration,head_res1,head_res2]
            writer.writerow(data_header)
        data =  ([i2c_call_num]) + ([result1]) + ([result2]) + ([result3])+ ([result4])+ ([result5])+ ([result6])+ ([result7])+ ([result8])+ ([result9])+ ([result10])+ ([result11])+ ([result12])+ ([result13])+ ([result14])+ ([result15])                       # for additional result columns, you can add result2,result3, ... to the end
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range (starting_mac,(starting_mac+num_of_macs)):
            ...
            i2c_path = os.getcwd()
            i2c_path = os.path.join(i2c_path,'i2c_files')
            i2c_path = os.path.join(i2c_path,'power_mthd4')        #specify what kind of test you are running
            ...
            for z, directory in enumerate(directories):
                ...
                for n in range(0,num_of_files_each_test):        ############for testing########
                    if n==0:
                          print('___________________please set the clock & offset OFF__________________')
                          time.sleep(15)
                    if n==1:
                          print('___________________please set the clock & offset ON__________________')
                          time.sleep(15)
                    rst = []                        
                    sda = []
                    scl = []
